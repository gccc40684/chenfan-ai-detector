...
function calculateAccuracy(
  humanResults: ReturnType<typeof batchDetect>,
  aiResults: ReturnType<typeof batchDetect>
): {
...
describe('Heuristic Detector Accuracy', () => {
  describe('Default Weights', () => {
    it('should achieve >= 80% accuracy on Chinese text', () => {
      const humanResults = batchDetect(humanTextsChinese);
      const aiResults = batchDetect(aiTextsChinese);
      const accuracy = calculateAccuracy(humanResults, aiResults);

      console.log('\n=== Chinese Text Detection (Default Weights) ===');
      console.log(`Human Accuracy: ${(accuracy.humanAccuracy * 100).toFixed(1)}%`);
      console.log(`AI Accuracy: ${(accuracy.aiAccuracy * 100).toFixed(1)}%`);
      console.log(`Overall Accuracy: ${(accuracy.overallAccuracy * 100).toFixed(1)}%`);
      console.log(
        'Human Scores:',
        accuracy.humanScores.map(s => s.toFixed(3))
      );
      console.log(
        'AI Scores:',
        accuracy.aiScores.map(s => s.toFixed(3))
      );

      expect(accuracy.overallAccuracy).toBeGreaterThanOrEqual(0.8);
    });

    it('should achieve >= 80% accuracy on English text', () => {
      const humanResults = batchDetect(humanTextsEnglish);
      const aiResults = batchDetect(aiTextsEnglish);
      const accuracy = calculateAccuracy(humanResults, aiResults);

      console.log('\n=== English Text Detection (Default Weights) ===');
      console.log(`Human Accuracy: ${(accuracy.humanAccuracy * 100).toFixed(1)}%`);
      console.log(`AI Accuracy: ${(accuracy.aiAccuracy * 100).toFixed(1)}%`);
      console.log(`Overall Accuracy: ${(accuracy.overallAccuracy * 100).toFixed(1)}%`);
      console.log(
  ...
  describe('Optimized Weights', () => {
    ...
    it('should achieve >= 85% accuracy on Chinese text with optimized weights', () => {
      const humanResults = humanTextsChinese.map(text => detectAI(text, optimizedConfig));
      const aiResults = aiTextsChinese.map(text => detectAI(text, optimizedConfig));
      const accuracy = calculateAccuracy(humanResults, aiResults);

      console.log('\n=== Chinese Text Detection (Optimized Weights) ===');
      console.log(`Human Accuracy: ${(accuracy.humanAccuracy * 100).toFixed(1)}%`);
      console.log(`AI Accuracy: ${(accuracy.aiAccuracy * 100).toFixed(1)}%`);
      console.log(`Overall Accuracy: ${(accuracy.overallAccuracy * 100).toFixed(1)}%`);
      console.log(
      ...
      expect(accuracy.overallAccuracy).toBeGreaterThanOrEqual(0.85);
    });

    it('should achieve >= 80% accuracy on English text with optimized weights', () => {
      const humanResults = humanTextsEnglish.map(text => detectAI(text, optimizedConfig));
      const aiResults = aiTextsEnglish.map(text => detectAI(text, optimizedConfig));
      const accuracy = calculateAccuracy(humanResults, aiResults);

      console.log('\n=== English Text Detection (Optimized Weights) ===');
      console.log(`Human Accuracy: ${(accuracy.humanAccuracy * 100).toFixed(1)}%`);
      console.log(`AI Accuracy: ${(accuracy.aiAccuracy * 100).toFixed(1)}%`);
      console.log(`Overall Accuracy: ${(accuracy.overallAccuracy * 100).toFixed(1)}%`);
      console.log(
  ...
  describe('Combined Language Accuracy', () => {
    it('should achieve >= 80% combined accuracy across all languages', () => {
      const allHuman = [...humanTextsChinese, ...humanTextsEnglish];
      const allAI = [...aiTextsChinese, ...aiTextsEnglish];

      const humanResults = batchDetect(allHuman);
      const aiResults = batchDetect(allAI);
      const accuracy = calculateAccuracy(humanResults, aiResults);

      console.log('\n=== Combined Language Detection ===');
      console.log(`Total Human Samples: ${allHuman.length}`);
      console.log(`Total AI Samples: ${allAI.length}`);
      console.log(`Human Accuracy: ${(accuracy.humanAccuracy * 100).toFixed(1)}%`);
      console.log(`AI Accuracy: ${(accuracy.aiAccuracy * 100).toFixed(1)}%`);
      console.log(`Overall Accuracy: ${(accuracy.overallAccuracy * 100).toFixed(1)}%`);
