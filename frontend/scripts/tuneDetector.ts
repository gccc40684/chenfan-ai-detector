 ...
 * 运行: npx tsx scripts/tuneDetector.ts
 */
...
function generateWeightGrid(): Array<Partial<DetectorConfig>> {
  const weights: Array<Partial<DetectorConfig>> = [];
  const step = 0.05;
...
function main() {
  ...
  printResult(evaluateConfig(baselineConfig));
  ...
  for (const baseConfig of grid) {
    for (const config of generateThresholdVariations(baseConfig)) {
